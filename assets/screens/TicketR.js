...
import Styles from "../styles/TicketR";
    ...
    await fetch("http://54.207.110.207:443/api/informacionTicketC", {
      ...
      .then((responseJsonFromServer) => {
        ...
        if (responseJsonFromServer.cod === 200) {
          this.setState({
            datos: responseJsonFromServer.response.enRuta,
            fontLoaded:true
          });
        }
      })
      ...
      return (
        <View style={Styles.fondo}>
          <View style={Styles.cont1}>
            <Text style={Styles.txtBtn}>Ticket En Ruta</Text>
          ...
          <View style={Styles.contenedorP}>
            ...
            {this.state.datos.length == 0 ? (
              <Text>NO TIENES TICKETS EN REPARTO</Text>
            ):(
              <View></View>
            )}
            ...
            {this.state.datos.map((item, index) => {
                return (
                  <TouchableOpacity 
                    style={Styles.row} 
                    key={item.idTicket} 
                    onPress={(event) => { this.props.navigation.navigate("aDetalleTicket", { id: item.idTicket,index:3 }) }}>
