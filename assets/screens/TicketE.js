...
import Styles from "../styles/TicketE";
...
export default class Menu extends Component {
  ...
  async ticket() {
    await fetch("http://54.207.110.207:443/api/informacionTicketC", {
      method: "POST",
      headers: {
      ...
      .then((responseJsonFromServer) => {
        ...
        if (responseJsonFromServer.cod === 200) {
          this.setState({
            datos: responseJsonFromServer.response.listoEntregar,
            fontLoaded: true
          });
        }
      })
      ...
      return (
        <View style={Styles.fondo}>
          <View style={Styles.cont1}>
            <Text style={Styles.txtBtn}>Ticket Espera Envio</Text>
          ...
          <View style={Styles.contenedorP}>
            ...
            >
              ...
              {this.state.datos.length == 0 ? (
                <Text>NO TIENES TICKETS LISTO PARA ENTREGAR</Text>
              ) : (
                <View></View>
              )}
              ...
              {this.state.datos.map((item, index) => {
                return (
                  <TouchableOpacity
                    style={Styles.row}
                    key={item.idTicket}
                    onPress={(event) => { this.props.navigation.navigate("aDetalleTicket", { id: item.idTicket,index:2 }) }}>
                    <Fontisto name="ticket-alt" size={24} color="black" />
                    <Text style={Styles.txtDesc10}> ${item.total}</Text>
                    <AntDesign
                      name="right"
                      size={24}
                      color="black"
